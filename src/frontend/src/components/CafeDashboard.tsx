...
export default function CafeDashboard() {
  ...
  const handleUpdateFreeCups = async () => {
    const cups = Number.parseInt(freeCups);
    ...
    cuppings && cuppings.length > 0
      ? {
          ...
          flavor:
            ...
            cuppings.length,
          aftertaste:
            cuppings.reduce((sum, c) => sum + c.scores.aftertaste, 0) /
            cuppings.length,
          acidity:
            cuppings.reduce((sum, c) => sum + c.scores.acidity, 0) /
            cuppings.length,
          body:
            cuppings.reduce((sum, c) => sum + c.scores.body, 0) /
            cuppings.length,
          balance:
            cuppings.reduce((sum, c) => sum + c.scores.balance, 0) /
            cuppings.length,
          uniformity:
            cuppings.reduce((sum, c) => sum + c.scores.uniformity, 0) /
            cuppings.length,
          sweetness:
            cuppings.reduce((sum, c) => sum + c.scores.sweetness, 0) /
            cuppings.length,
          cleanCup:
            cuppings.reduce((sum, c) => sum + c.scores.cleanCup, 0) /
            cuppings.length,
          overall:
            cuppings.reduce((sum, c) => sum + c.scores.overall, 0) /
            cuppings.length,
        }
      : null;
    ...
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-4 text-center text-sm text-muted-foreground">
              Based on {cuppings?.length || 0} cupping
              {cuppings?.length !== 1 ? "s" : ""}
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Set the number of available free cups for today
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex gap-2">
            ...
            />
            <Button
              onClick={handleUpdateFreeCups}
              disabled={updateFreeCups.isPending}
            >
              Update
            </Button>
          </div>
            ...
            <Dialog open={showAddCoffee} onOpenChange={setShowAddCoffee}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Add a new coffee to your inventory
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={newCoffee.id}
                      onChange={(e) =>
                        setNewCoffee({ ...newCoffee, id: e.target.value })
                      }
