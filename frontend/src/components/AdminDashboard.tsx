...
import { Users, Coffee, QrCode, Store, TrendingUp, AlertCircle } from 'lucide-react';
    ...
    return (
      <Card className="border-destructive">
        <CardContent className="flex items-center gap-3 pt-6">
          ...
          <p className="text-destructive">Failed to load daily statistics. Please try again.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Daily activity for the last 7 days</p>
        </div>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              ...
              />
              <Legend />
              <Bar dataKey="newUsers" name="New Users" fill="#c2855a" radius={[3, 3, 0, 0]} />
              <Bar dataKey="cuppings" name="Cuppings" fill="#8b5e3c" radius={[3, 3, 0, 0]} />
              <Bar
                ...
                radius={[3, 3, 0, 0]}
              />
              <Bar dataKey="newCafes" name="New Cafes" fill="#6b4226" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
              ...
              {chartData.map((day) => (
                <TableRow key={day.date}>
                  ...
                  <TableCell className="text-right">
                    {day.newUsers > 0 ? (
                      <span className="font-semibold text-primary">{day.newUsers}</span>
                    ) : (
                  ...
                  <TableCell className="text-right">
                    {day.cuppings > 0 ? (
                      <span className="font-semibold text-primary">{day.cuppings}</span>
                    ) : (
                      <span className="text-muted-foreground">0</span>
                    )}
                  </TableCell>
                  <TableCell className="text-right">
                    {day.qrRedemptions > 0 ? (
                      <span className="font-semibold text-primary">{day.qrRedemptions}</span>
                    ) : (
                      <span className="text-muted-foreground">0</span>
                    )}
                  </TableCell>
                  <TableCell className="text-right">
                    {day.newCafes > 0 ? (
                      <span className="font-semibold text-primary">{day.newCafes}</span>
                    ) : (
                      <span className="text-muted-foreground">0</span>
                    )}
                  </TableCell>
